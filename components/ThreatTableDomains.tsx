...
import { Eye, Bell, ExternalLink, CheckCircle2, AlertTriangle, Shield, TrendingUp, EyeIcon, RefreshCcw, FileArchive } from "lucide-react";
import { IDomains } from "@/lib/types";
...
interface IThreatTableDomainsProps {
...
export default function ThreatTableDomains({ brandId, verifiedThreatFilter, notifiedThreatFilter, archivingThreatFilter, reloadFilter, newThreat, logged, data, count, countResults, countAllThreats, countAllAccesses, allThreats, endDate, startDate }: IThreatTableDomainsProps) {
    const [updateStateDomains, setUpdateStateDomains] = useState<boolean>(false);
    const [loading, setLoading] = useState(false);
    const [dataDomains, setDataDomains] = useState<IDomains[] | undefined>(data);
    const [countDomains, setCountDomains] = useState<number | undefined>(count);
    const [countResultsDomains, setCountResultsDomains] = useState<number | undefined>(countResults);
    const [takeDomains] = useState<number>(10);
    const [pageDomains, setPageDomains] = useState(1);
    const [isDetailDialogOpen, setIsDetailDialogOpen] = useState(false);
    const [selectedThreat, setSelectedThreat] = useState<IDomains | null>(null);
    ...
    useEffect(() => {
        ...
        async function getDomainsThreats() {
            const response = await makeRequest("get", `/domains/${brandId}?skip=${pageDomains == 1 ? 0 : (pageDomains - 1) * takeDomains}&take=${takeDomains}&verified=${verifiedThreatFilter}&notified=${notifiedThreatFilter}&archiving=${archivingThreatFilter}&startDate=${startDate}&endDate=${endDate}`);

            if ("status" in response) {
                return;
            } else {
                setDataDomains(response.domains);
                setCountDomains(response.count);
                setCountResultsDomains(response.domains.length);
            }
        }

        getDomainsThreats();
    }, [pageDomains, verifiedThreatFilter, reloadFilter, notifiedThreatFilter, updateStateDomains, startDate, endDate]);
    ...
    const handleChangePaginationDomains = (event: React.ChangeEvent<unknown>, value: number) => {
        setPageDomains(value);
    };

    /** Abre o modal com os detalhes da ameaça */
    const openDetailDialog = (threat: IDomains) => {
    ...
    const archiveThreat = async (id: string, archiving: string) => {
        ...
        const response = await makeRequest("put", `/domains-archive/${id}`, { archiving });
        ...
        if (response.status === 200) {
            ...
            setUpdateStateDomains(!updateStateDomains);
    ...
    const sendNotification = async (threat: IDomains) => {
        setLoading(true);
    ...
    const handleSendNotification = async (Threat: IDomains) => {
    ...
    const markAsVerified = async (id: string, verifiedThreat: string) => {
        ...
        const response = await makeRequest("put", `/domains/${id}`, { verifiedThreat });
    ...
    const getDomainInfo = async (id: string) => {
        setSpinning(true);

        const response = await makeRequest("put", `/domains-info/${id}`);
        ...
        if (response.status === 400) {
            toast.info("Nada encontramos", {
                description: `Informações não encontradas, tente mais tarde!`,
            });
        }
            ...
            {dataDomains !== undefined ? (
                <>
                                    ...
                                    <DialogHeader className="mt-6">
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1 min-w-0 text-left">
                                                <DialogTitle className="text-2xl break-words">Ocorrência de Possível Ameaça Suspeita em {extractRootDomain(selectedThreat?.domain || "")}</DialogTitle>
                                            </div>
                                        </div>
                                    ...
                                    <div className="space-y-6 mt-4">
                                                ...
                                                <>
                                                    <div>
                                                        ...
                                                        <Button variant="outline" size="sm" asChild>
                                                            <a href={`http://${selectedThreat.domain}`} target="_blank" rel="noopener noreferrer">
                                                                <ExternalLink className="mr-2 h-4 w-4" />
                                                                {selectedThreat.domain.length > 30 ? selectedThreat.domain.slice(0, 30) + "..." : `http://${selectedThreat.domain}`}
                                                            </a>
                                                        </Button>
                                                    </div>
                                                    <Badge variant="default" className={selectedThreat?.status ? "bg-success text-white" : "bg-destructive text-white"}>
                                                        {selectedThreat?.status ? "Site no ar" : "Site inativo"}
                                                    </Badge>
                                                </>
                                            )}
                                            ...
                                            {selectedThreat?.details ? (
                                                <div>
                                                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2 gap-2">
                                                        <p className="text-sm text-muted-foreground">Informações do Domínio</p>
                                                                ...
                                                                <div className="space-y-6 pb-10">
                                                                    <div className="grid grid-cols-2 gap-8">
                                                                        <div className="col-span-2 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Registrador</span>
                                                                            <div className="flex items-center gap-2 mt-1">
                                                                                <p className="text-base">{selectedThreat?.details?.registrar || "Não encontrado"}</p>
                                                                            </div>
                                                                        </div>

                                                                        <div className="col-span-2 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Organização do proprietário</span>
                                                                            <p className="text-base mt-1">{selectedThreat?.details?.registrant_organization || "Não encontrada"}</p>
                                                                        </div>

                                                                        <div className="col-span-2 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">ID de registro do domínio</span>
                                                                            <p className="text-base mt-1">{selectedThreat?.details?.registry_domain_id || "Não encontrado"}</p>
                                                                        </div>

                                                                        <div className="space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Nome do domínio</span>
                                                                            <p className="text-base mt-1 flex items-center gap-1">
                                                                                {selectedThreat?.details?.domain_name || "Não encontrado"}
                                                                                {selectedThreat?.details?.domain_name && (
                                                                                    <a href={`https://${selectedThreat.domain}`} target="_blank" rel="noopener noreferrer">
                                                                                        <ExternalLink className="mr-2 h-4 w-4" />
                                                                                    </a>
                                                                                )}
                                                                            </p>
                                                                        ...
                                                                        <div className="col-span-2 sm:col-span-1 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Site do registrador</span>
                                                                            <div className="flex items-center gap-2 mt-1">
                                                                                <p className="text-base flex items-center gap-1">
                                                                                    {selectedThreat?.details?.registrar_url || "Não encontrado"}
                                                                                    {selectedThreat?.details?.registrar_url && (
                                                                                        <a href={`${selectedThreat?.details?.registrar_url}`} target="_blank" rel="noopener noreferrer">
                                                                                            <ExternalLink className="mr-2 h-4 w-4" />
                                                                                        </a>
                                                                                    )}
                                                                                </p>
                                                                        ...
                                                                        <div className="col-span-2 sm:col-span-1 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Servidor WHOIS do registrador</span>
                                                                            <p className="text-base mt-1">{selectedThreat?.details?.registrar_whois_server || "Não encontrado"}</p>
                                                                        </div>

                                                                        <div className="col-span-2 sm:col-span-1 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">E-mail para denúncias</span>
                                                                            <p className="text-base mt-1">{selectedThreat?.details?.registrar_abuse_contact_email || "Não encontrado"}</p>
                                                                        </div>

                                                                        <div className="space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Telefone para denúncias</span>
                                                                            <p className="text-base mt-1">{selectedThreat?.details?.registrar_abuse_contact_phone || "Não encontrado"}</p>
                                                                        </div>

                                                                        <div className="col-span-2 sm:col-span-1 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Data de criação</span>
                                                                            {selectedThreat?.details?.creation_date ? <p className="text-base mt-1">{new Date(selectedThreat?.details?.creation_date || "").toLocaleDateString("pt-BR")}</p> : <p className="text-base mt-1">Não encontrada</p>}
                                                                        </div>

                                                                        <div className="col-span-2 sm:col-span-1 space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Última atualização</span>
                                                                            {selectedThreat?.details?.updated_date ? <p className="text-base mt-1">{new Date(selectedThreat?.details?.updated_date || "").toLocaleDateString("pt-BR")}</p> : <p className="text-base mt-1">Não encontrada</p>}
                                                                        </div>

                                                                        <div className="space-y-1">
                                                                            <span className="text-muted-foreground text-xs uppercase">Data de expiração</span>
                                                                            {selectedThreat?.details?.expiration_date ? <p className="text-base mt-1">{new Date(selectedThreat?.details?.expiration_date || "").toLocaleDateString("pt-BR")}</p> : <p className="text-base mt-1">Não encontrada</p>}
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                ...
                                                <>
                                                    <div className="flex items-center justify-between mb-2">
                                                        <p className="text-sm text-muted-foreground">Busque as Informações do Domínio</p>
                                                        <div className="flex gap-2 flex items-center justify-center">
                                                            <Button variant="outline" size="sm" disabled={spinning ? true : false} className="p-2 rounded bg-muted-foreground/40 text-white hover:bg-muted-foreground/30 flex items-center gap-2" onClick={() => getDomainInfo(selectedThreat?.id || "")}>
                                                                <RefreshCcw className={`h-5 w-5 ${spinning ? "animate-spin" : ""}`} />
                                                                Buscar Informações
                                                            </Button>
                                                        </div>
                                                    </div>
                                        ...
                                        <div className="border-t pt-4 space-y-2">
                                            <div className="flex justify-between text-sm">
                                                ...
                                                <span className="font-medium">{extractRootDomain(selectedThreat?.domain || "")}</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                    ...
                    <div className="grid gap-4 grid-cols-[repeat(auto-fit,minmax(250px,1fr))] mb-8">
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-sm font-medium">Ameaças em Domínios</CardTitle>
                                <AlertTriangle className="h-4 w-4 text-destructive" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-destructive">{countDomains}</div>
                                <p className="text-xs text-muted-foreground">Ameaças detectadas somente em Domínios</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-success">{countDomains && countAllThreats && Math.round((countDomains / countAllThreats) * 100)}%</div>
                                <p className="text-xs text-muted-foreground">Percentual de Detecção em Domínios</p>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="no-export">{dataDomains && countDomains !== 0 && countResultsDomains !== 0 && <Paginations handleChangePagination={handleChangePaginationDomains} count={countDomains} take={takeDomains} />}</div>
                    <ConfirmationDialog />
                </>
            ) : (
                <TableSkeleton />
