...
interface VariationDomainBankEditor {
    variationDomainBank: string[][];
    onChange: (updatedDomainBank: string[][]) => void;
...
export function VariationDomainBankEditor({ variationDomainBank, onChange, variationsDomainName, setVariationsDomainName, setIndexGroupTagsDomain }: VariationDomainBankEditor) {
    ...
    const [selectedIndex, setSelectedIndex] = useState<number | null>(() => {
        if (!Array.isArray(variationsDomainName) || variationsDomainName.length === 0) return null;
        const idx = variationDomainBank.findIndex((group) => group.length === variationsDomainName.length && group.every((v, i) => v === variationsDomainName[i]));
    ...
    const handleRemoveTag = (groupIndex: number, tagIndex: number) => {
        const updated = variationDomainBank.map((group, gi) => (gi === groupIndex ? group.filter((_, ti) => ti !== tagIndex) : group));
        onChange(updated);
    };
    ...
    const handleAddTag = (groupIndex: number) => {
        ...
        const updated = variationDomainBank.map((group, gi) => (gi === groupIndex ? [...group, tag] : group));
        onChange(updated);
    ...
    return (
        <div className="flex flex-col items-center justify-start gap-3 p-2 max-h-[350px] overflow-auto">
            {variationDomainBank.map((content, groupIndex) => {
                ...
                return (
                    <div
                        ...
                        style={{
                            border: selected ? "solid 5px var(--success)" : "solid 1px var(--border)",
                        }}
                    >
