...
import { IGeneralWeb } from "@/lib/types";
...
interface IThreatTableGeneralWebProps {
...
export default function ThreatTableGeneralWeb({ brandId, verifiedThreatFilter, notifiedThreatFilter, archivingThreatFilter, reloadFilter, newThreat, logged, data, count, countResults, countAllThreats, countAllAccesses, allThreats, endDate, startDate }: IThreatTableGeneralWebProps) {
    const [updateStateGeneralWeb, setUpdateStateGeneralWeb] = useState<boolean>(false);
    const [loading, setLoading] = useState(false);
    const [dataGeneralWeb, setDataGeneralWeb] = useState<IGeneralWeb[] | undefined>(data);
    const [countGeneralWeb, setCountGeneralWeb] = useState<number | undefined>(count);
    const [countResultsGeneralWeb, setCountResultsGeneralWeb] = useState<number | undefined>(countResults);
    const [takeGeneralWeb] = useState<number>(10);
    const [pageGeneralWeb, setPageGeneralWeb] = useState(1);
    const [isDetailDialogOpen, setIsDetailDialogOpen] = useState(false);
    const [selectedThreat, setSelectedThreat] = useState<IGeneralWeb | null>(null);
    ...
    /** Busca as ameaças da (Web em geral) */
    useEffect(() => {
        if (render) {
            setRender(false);
            return;
        ...
        async function getGeneralWebThreats() {
            const response = await makeRequest("get", `/general-web/${brandId}?skip=${pageGeneralWeb == 1 ? 0 : (pageGeneralWeb - 1) * takeGeneralWeb}&take=${takeGeneralWeb}&verified=${verifiedThreatFilter}&notified=${notifiedThreatFilter}&archiving=${archivingThreatFilter}&startDate=${startDate}&endDate=${endDate}`);

            if ("status" in response) {
                return;
            } else {
                setDataGeneralWeb(response.generalWeb);
                setCountGeneralWeb(response.count);
                setCountResultsGeneralWeb(response.generalWeb.length);
            }
        }

        getGeneralWebThreats();
    }, [pageGeneralWeb, verifiedThreatFilter, reloadFilter, notifiedThreatFilter, updateStateGeneralWeb, startDate, endDate]);
    ...
    const handleChangePaginationGeneralWeb = (event: React.ChangeEvent<unknown>, value: number) => {
        setPageGeneralWeb(value);
    };

    /** Abre o modal com os detalhes da ameaça */
    const openDetailDialog = (threat: IGeneralWeb) => {
    ...
    const archiveThreat = async (id: string, archiving: string) => {
        ...
        const response = await makeRequest("put", `/general-web-archive/${id}`, { archiving });
        ...
        if (response.status === 200) {
            ...
            setUpdateStateGeneralWeb(!updateStateGeneralWeb);
    ...
    const sendNotification = async (threat: IGeneralWeb) => {
        setLoading(true);
    ...
    const handleSendNotification = async (Threat: IGeneralWeb) => {
    ...
    const markAsVerified = async (id: string, verifiedThreat: string) => {
        ...
        const response = await makeRequest("put", `/general-web/${id}`, { verifiedThreat });
            ...
            {dataGeneralWeb !== undefined ? (
                <>
                    ...
                    <div className="grid gap-4 grid-cols-[repeat(auto-fit,minmax(250px,1fr))] mb-8">
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-sm font-medium">Ameaças na Web</CardTitle>
                                <AlertTriangle className="h-4 w-4 text-destructive" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-destructive">{countGeneralWeb}</div>
                                <p className="text-xs text-muted-foreground">Ameaças detectadas somente na Web</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-success">{countGeneralWeb && countAllThreats && Math.round((countGeneralWeb / countAllThreats) * 100)}%</div>
                                <p className="text-xs text-muted-foreground">Percentual de Detecção na Web</p>
                            </CardContent>
                        </Card>
                    </div>
                            ...
                            <Table className="border-b border-l border-r rounded-lg border-border">
                                ...
                                <TableBody>
                                    {countGeneralWeb === 0 ? (
                                        ...
                                        dataGeneralWeb?.map((threat) => (
                                            <TableRow key={threat.id} className="group transition-all duration-900 ease-out animate-[slideDown_0.8s_ease-out]">
                    ...
                    <div className="no-export">{dataGeneralWeb && countGeneralWeb !== 0 && countResultsGeneralWeb !== 0 && <Paginations handleChangePagination={handleChangePaginationGeneralWeb} count={countGeneralWeb} take={takeGeneralWeb} />}</div>
                    <ConfirmationDialog />
                </>
            ) : (
                <TableSkeleton />
