...
import { ILogoComparisons } from "@/lib/types";
...
interface IThreatTableLogoComparisonsProps {
    ...
    logged: boolean | null;
    data: ILogoComparisons[] | undefined;
...
export default function ThreatTableLogoComparisons({ brandId, verifiedThreatFilter, notifiedThreatFilter, archivingThreatFilter, reloadFilter, newThreat, logged, data, count, countResults, countAllThreats, countAllAccesses, allThreats, endDate, startDate }: IThreatTableLogoComparisonsProps) {
    const [updateStateLogoComparisons, setUpdateStateLogoComparisons] = useState<boolean>(false);
    const [loading, setLoading] = useState(false);
    const [dataLogoComparisons, setDataLogoComparisons] = useState<ILogoComparisons[] | undefined>(data);
    const [countLogoComparisons, setCountLogoComparisons] = useState<number | undefined>(count);
    const [countResultsLogoComparisons, setCountResultsLogoComparisons] = useState<number | undefined>(countResults);
    const [takeLogoComparisons] = useState<number>(10);
    const [pageLogoComparisons, setPageLogoComparisons] = useState(1);
    const [isDetailDialogOpen, setIsDetailDialogOpen] = useState(false);
    const [selectedThreat, setSelectedThreat] = useState<ILogoComparisons | null>(null);
    ...
    /** Busca as ameaças da (Comparações de Logo) */
    useEffect(() => {
        if (render) {
            setRender(false);
            return;
        ...
        async function getLogoComparisonsThreats() {
            const response = await makeRequest("get", `/logo-comparisons/${brandId}?skip=${pageLogoComparisons == 1 ? 0 : (pageLogoComparisons - 1) * takeLogoComparisons}&take=${takeLogoComparisons}&verified=${verifiedThreatFilter}&notified=${notifiedThreatFilter}&archiving=${archivingThreatFilter}&startDate=${startDate}&endDate=${endDate}`);

            if ("status" in response) {
                return;
            } else {
                setDataLogoComparisons(response.logoComparisons);
                setCountLogoComparisons(response.count);
                setCountResultsLogoComparisons(response.logoComparisons.length);
            }
        }

        getLogoComparisonsThreats();
    }, [pageLogoComparisons, verifiedThreatFilter, reloadFilter, notifiedThreatFilter, updateStateLogoComparisons, startDate, endDate]);

    /** Resetar para a primeira página quando necessário */
    useEffect(() => {
        setPageLogoComparisons(1);
    }, [verifiedThreatFilter, notifiedThreatFilter]);

    /** Controla a paginação dos ativos de monitoramento (Marcas) */
    const handleChangePaginationLogoComparisons = (event: React.ChangeEvent<unknown>, value: number) => {
        setPageLogoComparisons(value);
    };

    /** Abre o modal com os detalhes da ameaça */
    const openDetailDialog = (threat: ILogoComparisons) => {
    ...
    const archiveThreat = async (id: string, archiving: string) => {
        ...
        const response = await makeRequest("put", `/logo-comparisons-archive/${id}`, { archiving });
        ...
        if (response.status === 200) {
            ...
            setUpdateStateLogoComparisons(!updateStateLogoComparisons);
    ...
    const sendNotification = async (threat: ILogoComparisons) => {
        setLoading(true);
    ...
    const handleSendNotification = async (Threat: ILogoComparisons) => {
    ...
    const markAsVerified = async (id: string, verifiedThreat: string) => {
        ...
        const response = await makeRequest("put", `/logo-comparisons/${id}`, { verifiedThreat });
            ...
            {dataLogoComparisons !== undefined ? (
                <>
                                ...
                                <>
                                    <DialogHeader className="mt-6">
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1 min-w-0 text-left">
                                                <DialogTitle className="text-2xl break-words">Ameaça de Colidência de Imagem Suspeita em {extractRootDomain(selectedThreat?.sourceLink || "")}</DialogTitle>
                                                <div className="text-sm bg-muted border-1 border-border pt-6 pb-6 pl-5 pr-3 rounded-md mt-4">{handleMarkdown(selectedThreat?.about ?? "")}</div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="space-y-6 mt-4">
                                        <div className="space-y-4">
                                            {selectedThreat?.sourceLink && (
                                                <div>
                                                    <p className="text-sm text-muted-foreground mb-2">Visitar link</p>
                                                    <Button variant="outline" size="sm" asChild>
                                                        <a href={selectedThreat.sourceLink} target="_blank" rel="noopener noreferrer">
                                                            <ExternalLink className="mr-2 h-4 w-4" />
                                                            {selectedThreat.sourceLink.length > 30 ? selectedThreat.sourceLink.slice(0, 30) + "..." : selectedThreat.sourceLink}
                                                        </a>
                                                    </Button>
                                                </div>
                                            )}

                                            {selectedThreat?.logo_url && selectedThreat?.occurrence_url && (
                                                <div>
                                                    ...
                                                    <div style={{ height: openImage ? `${heightImage}px` : "0px", transition: "height 300ms ease" }} className="overflow-hidden mt-2">
                                                        <div ref={divRef} className="p-4 border rounded bg-muted">
                                                            {selectedThreat?.brand?.assetType === "brand" ? (
                                                                <a href={selectedThreat.logo_url || ""} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2">
                                                            ...
                                                            <div className="relative aspect-video w-full rounded-lg border bg-muted mb-4 flex items-center justify-center">
                                                                {!image1Error ? (
                                                                    <img src={selectedThreat.logo_url} alt="Preview" className="object-contain w-full h-full" onError={() => setImage1Error(true)} />
                                                                ) : (
                                                            ...
                                                            <a href={selectedThreat.occurrence_url || ""} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2">
                                                            ...
                                                            <div className="relative aspect-video w-full rounded-lg border bg-muted flex items-center justify-center">
                                                                {!image2Error ? (
                                                                    <img src={selectedThreat.occurrence_url} alt="Preview" className="object-contain w-full h-full" onError={() => setImage2Error(true)} />
                                                                ) : (
                                                                    <div className="flex flex-col items-center justify-center gap-2">
                                                                        <ImageOff size={50} className="text-muted-foreground" />
                                                                        <div className="text-sm text-muted-foreground text-center px-4">Não foi possível carregar a imagem</div>
                                        ...
                                        <div className="border-t pt-4 space-y-2">
                                            <div className="flex justify-between text-sm">
                                                ...
                                                <span className="font-medium">{extractRootDomain(selectedThreat?.sourceLink || "")}</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                    ...
                    <div className="grid gap-4 grid-cols-[repeat(auto-fit,minmax(250px,1fr))] mb-8">
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-sm font-medium">Ameaças nos Logos</CardTitle>
                                <AlertTriangle className="h-4 w-4 text-destructive" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-destructive">{countLogoComparisons}</div>
                                <p className="text-xs text-muted-foreground">Ameaças detectadas somente nos Logos</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-success">{countLogoComparisons && countAllThreats && Math.round((countLogoComparisons / countAllThreats) * 100)}%</div>
                                <p className="text-xs text-muted-foreground">Percentual de Detecção nos Logos</p>
                            </CardContent>
                        </Card>
                    </div>
                            ...
                            <Table className="border-b border-l border-r rounded-lg border-border">
                                ...
                                <TableBody>
                                    {countLogoComparisons === 0 ? (
                                        ...
                                        dataLogoComparisons?.map((threat) => (
                                            <TableRow key={threat.id} className="transition-all duration-900 ease-out animate-[slideDown_0.8s_ease-out]">
                                                <TableCell className="font-medium pl-5 relative">
                                                    {threat?.occurrence_url && !brokenImages.has(threat.occurrence_url) ? (
                                                        <div className="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center bg-muted border border-border">
                                                            <img src={threat.occurrence_url} alt="Preview" className="w-full h-full object-cover" onError={() => handleImageError(threat?.occurrence_url || "")} />
                                                        </div>
                                                    ) : (
                                                        <div className="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center bg-muted border border-border">
                                                ...
                                                <TableCell className="font-medium pl-5">
                                                    <Badge variant="outline">{extractRootDomain(threat?.sourceLink || "")}</Badge>
                                                </TableCell>
                                                {logged && (
                                                    <TableCell>
                    ...
                    <div className="no-export">{dataLogoComparisons && countLogoComparisons !== 0 && countResultsLogoComparisons !== 0 && <Paginations handleChangePagination={handleChangePaginationLogoComparisons} count={countLogoComparisons} take={takeLogoComparisons} />}</div>
                    <ConfirmationDialog />
                </>
            ) : (
                <TableSkeleton />
